...
@Import({MysqlDataSourceConfig.class,CommonBatchAsyncConfig.class})
public class SimpleBatchAsyncConfig {
	...
	@Bean
	public Job myJob() {
		return jobBuilderFactory.get("myJob").start(myJobStep1()).preventRestart().listener(simpleJobExecutionListener()).build();
	}
